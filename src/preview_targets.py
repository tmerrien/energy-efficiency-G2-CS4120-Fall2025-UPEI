...
def main() -> None:
    ...
    print(
        f"[Regression] heating_load stats: min={y_reg.min():.3f}, max={y_reg.max():.3f}, mean={y_reg.mean():.3f}"
    )

    # --- PREVIEW classification w/ global median
    y_cls_all, info_all = make_classification_labels_from_hl(
        df, target_col="heating_load", threshold=None
    )
