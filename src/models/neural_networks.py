"""
Neural network models for energy efficiency prediction.

Implements MLP-based models with learning curve tracking for both
...
class MLPClassifierWithHistory(MLPClassifier):
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if X_val is not None and y_val is not None:
            # Use partial_fit to track validation loss per epoch
            # Note: sklearn's MLPClassifier doesn't directly support this,
            # so we approximate by evaluating after training
...
class MLPRegressorWithHistory(MLPRegressor):
    """
    MLP Regressor that tracks training history for learning curves.
    
    Extends sklearn's MLPRegressor to store loss history during training.
    """
...
def create_mlp_regressor(
    ...
    early_stopping: bool = True,
    validation_fraction: float = 0.1,
) -> MLPRegressorWithHistory:
    """
    Create an MLP regressor with sensible defaults for the energy efficiency dataset.
    ...
    Returns:
        Configured MLPRegressorWithHistory instance
    """
    return MLPRegressorWithHistory(
        hidden_layer_sizes=hidden_layer_sizes,
        activation='relu',
        solver='adam',
        alpha=alpha,
